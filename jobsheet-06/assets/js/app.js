...
function initHapusConfirm() {
    document.addEventListener("click", function (e) {
        // LATIHAN NO. 4: Cetak elemen yang diklik ke Console
...
//Fungsi Generik Fetch & Render Data (Latihan Jobsheet 6 No. 2)
async function muatDataGenerik(urlJson, tbodySelector, kolomArray) {
    ...
    try {
        ...
        data.forEach((item) => {
            ...
            let htmlKolom = "";
            kolomArray.forEach((kolom) => {
                htmlKolom += `<td>${item[kolom] ?? "-"}</td>`;
            });
            htmlKolom += `
                <td>
        ...
        tbody.innerHTML = `<tr><td colspan="10">Gagal memuat data: ${error.message}</td></tr>`;
    } finally {
